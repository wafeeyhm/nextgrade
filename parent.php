...
<head>
  ...
  <title>Portal Ibu Bapa - NextGrade Insights</title>
...
<div class="max-w-6xl w-full mx-auto flex flex-col gap-6">
  ...
  <header class="bg-white px-6 py-4 rounded-3xl shadow-sm border-2 border-slate-200 flex flex-wrap items-center justify-between gap-4">
    <div class="flex items-center gap-3">
      ...
      >
        <span>⬅️</span>
        <span class="font-black">Utama</span>
      </a>
      <div>
        <h1 class="text-xl md:text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
          <span>👨‍👩‍👧</span> Portal Ibu Bapa
        </h1>
        <p class="text-xs font-bold text-slate-400">Prestasi, Kemajuan & Analisis Pembelajaran Anak</p>
      </div>
    </div>

    <!-- Student Filter Dropdown -->
    <div class="flex items-center gap-2">
      <label for="filter-student-select" class="text-xs font-extrabold text-slate-500 uppercase">Pilih Murid:</label>
      <select 
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-sm flex items-center gap-4">
      ...
      <div>
        <span class="block text-xs font-black uppercase text-slate-400">Jumlah Sesi</span>
    ...
    <div class="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-sm flex items-center gap-4">
      ...
      <div>
        <span class="block text-xs font-black uppercase text-slate-400">Ketepatan Purata</span>
    ...
    <div class="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-sm flex items-center gap-4">
      ...
      <div>
        <span class="block text-xs font-black uppercase text-slate-400">Soalan Dijawab</span>
    ...
    <div class="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-sm flex items-center gap-4">
      ...
      <div>
        <span class="block text-xs font-black uppercase text-slate-400">Masa Belajar</span>
  ...
  <div class="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-slate-200 shadow-sm">
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
        <span>📊</span> Prestasi Mengikut Mata Pelajaran
      </h2>
      <span class="text-xs font-bold text-slate-400">Disimpan dalam Pangkalan Data MySQL</span>
    </div>

    <div id="subject-breakdown-list" class="space-y-4">
      <!-- Subject progress items rendered via API -->
      <div class="text-center py-6 text-slate-400 font-bold">Memuatkan pecahan subjek...</div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-emerald-200 shadow-sm">
      <h3 class="text-xl font-black text-emerald-800 mb-4 flex items-center gap-2">
        <span>🌟</span> Kekuatan Anak (Kefahaman Tinggi)
      </h3>
      <div id="strengths-list" class="space-y-3">
        <p class="text-sm font-bold text-slate-400">Belum ada rekod penilaian yang mencukupi.</p>
      </div>
    </div>

    <!-- Topics Needing Practice (Perlu Ulang Kaji) -->
    <div class="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-amber-200 shadow-sm">
      <h3 class="text-xl font-black text-amber-800 mb-4 flex items-center gap-2">
        <span>🎯</span> Cadangan Ulang Kaji & Fokus Tambahan
      </h3>
      <div id="needs-practice-list" class="space-y-3">
        <p class="text-sm font-bold text-slate-400">Semua topik berada pada tahap memuaskan!</p>
      </div>
    </div>
  ...
  <div class="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-slate-200 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
        <span>📜</span> Log Sesi Kuiz Terkini
      </h2>
      <span class="text-xs font-bold text-slate-400">20 Sesi Terakhir</span>
    </div>
      ...
      <table class="w-full text-left text-sm font-bold">
        <thead>
          <tr class="border-b-2 border-slate-200 text-slate-400 uppercase text-xs">
            <th class="py-3 px-3">Tarikh & Masa</th>
            <th class="py-3 px-3">Murid</th>
            <th class="py-3 px-3">Mata Pelajaran</th>
            <th class="py-3 px-3">Topik</th>
            <th class="py-3 px-3 text-center">Markah (10)</th>
            <th class="py-3 px-3 text-center">Peratus</th>
            <th class="py-3 px-3 text-right">Masa Diambil</th>
          </tr>
        </thead>
        <tbody id="sessions-table-body" class="divide-y divide-slate-100 text-slate-700">
          <tr>
            <td colspan="7" class="text-center py-8 text-slate-400">Belum ada sesi kuiz yang direkodkan.</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  async function loadParentData(student = '') {
    try {
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <div class="text-xs font-black text-slate-500">
              ...
              (${s.correct_answers} / ${s.questions_attempted} soalan)
            </div>
          </div>
        ...
        strList.innerHTML = data.insights.strengths.map(st => `
          <div class="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between text-sm">
            ...
            <span class="bg-emerald-600 text-white text-xs font-black px-2.5 py-1 rounded-lg">
              ${Math.round(st.avg_score)}% Betul
            </span>
          </div>
        `).join('');
      } else {
        strList.innerHTML = '<p class="text-sm font-bold text-slate-400 py-3">Tiada topik yang melebihi 70% setakat ini. Teruskan berusaha!</p>';
      }
      ...
      if (data.insights.needs_practice && data.insights.needs_practice.length > 0) {
        ...
        npList.innerHTML = '<p class="text-sm font-bold text-slate-400 py-3">Bagus sekali! Semua topik berada pada tahap penguasaan yang baik.</p>';
      }
      ...
      if (data.recent_sessions && data.recent_sessions.length > 0) {
        ...
        tbBody.innerHTML = '<tr><td colspan="7" class="text-center py-8 text-slate-400 font-bold">Belum ada sesi kuiz yang direkodkan.</td></tr>';
      }
